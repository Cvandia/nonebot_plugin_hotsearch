...
# 热搜条数dn
cnt = plugin_config.hotsearch_cnt
...
bzhan_hotsearch = on_command(
    "b站热搜", aliases={"B站热搜"}, permission=GROUP, priority=5, block=True
)
...
async def baiduresou(bot: Bot, event: GroupMessageEvent):
    ...
    if not data:
        await baidu_hotsearch.finish("获取失败")
...
async def zhihuresou(bot: Bot, event: GroupMessageEvent):
    ...
    if not data:
        await zhihu_hotsearch.finish("获取失败")
    data = data.groups()[1]
    msg_list = ["知乎热搜榜"]
        ...
        async with httpx.AsyncClient() as r:
            ...
            try:
                await bot.send_group_forward_msg(
                    group_id=group_id, messages=forward_msg
                )
            except:
                await bot.send(event=event, message="合并消息获取失败，稍后重新试试吧~")
    except Exception as e:
        await bot.send(message=f"获取B站热搜失败，error:{e}", event=event)
